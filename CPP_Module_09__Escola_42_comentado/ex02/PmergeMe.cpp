...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/21 11:03:55 by maclara-          #+#    #+#             */
/*   Updated: 2023/09/02 12:05:13 by maclara-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::vector<int> PmergeMe::mergeInsertionSort(Comparator comp, const std::vector<int>& coll) {
    if (coll.size() < 2) {
        ...
        ComparatorSort compSort(comp);
        std::sort(sortedPairs.begin(), sortedPairs.end(), compSort);
...
std::deque<int> PmergeMe::mergeInsertionSort_deque(Comparator comp, const std::deque<int>& coll) {
    if (coll.size() < 2) {
        ...
        for (std::deque<int>::iterator it = order.begin(); it != order.end(); ++it) {
            binaryInsertLambda(*it, comp, mainChain, aPositions, pendingElements);
        }
